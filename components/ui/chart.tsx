...
/**
 * shadcn's chart primitives, with TWO deliberate edits for the dark re-skin
 * (issue #63, map #59). Both are easy to lose to a `shadcn add chart`, which
 * would restore the registry file wholesale — re-apply them if that happens:
 *
 * 1. `ChartContainer`'s grid stroke is `--chart-grid`, not `border/50`. The
 *    registry value is `--border` at half strength; on this palette that is
 *    4% white on a 0.185 card, i.e. a grid you cannot see.
 * 2. `ChartTooltipContent` sits on `--popover` with a full-strength
 *    `--border`, not on `--background` with `border/50`. The tooltip floats
...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "grid min-w-32 items-start gap-1.5 rounded-lg border border-border bg-popover px-2.5 py-1.5 text-xs shadow-xl",
        className,
      )}
    >
